...
calc_fitness <- function(t_matrix, p_matrix, strat_vec, n_steps = 200, start_pos = c(1, 1), start_score = 0, v = FALSE){
  ...
  while(cstep <= n_steps){
    ...
    strat <- find_strat(find_state(p_matrix, view), strat_vec)
    # initialize a 'score' of 'fitness'
    score <- start_score
    # carry out strategy - move, do nothing, pick up target, etc.
    # positions: 1 = N, 2 = S, 3 = W, 4 = E, 5 = nothing, 6 = pick up target
    if(v){
      sprintf("Local view: %d", view)
      sprintf("Strategy selected: %d", strat)
    }
